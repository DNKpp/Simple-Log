...
SCENARIO("test nullptr predicate", "[nullptr-predicate]")
{
	WHEN("testing with non-nullptr")
	{
		GIVEN("as void*")
		{
			THEN("returns true")
			{
				int i = 0;
				REQUIRE(pred::notNullptr(static_cast<void*>(&i)));
			}
		}
		...
		GIVEN("as const void*")
		{
			...
			{
				int i = 0;
				REQUIRE(pred::notNullptr(static_cast<const void*>(&i)));
			}
		}
	...
	WHEN("testing with nullptr")
	{
		GIVEN("as void*")
		{
			THEN("returns false")
			{
				REQUIRE_FALSE(pred::notNullptr(static_cast<void*>(nullptr)));
			}
		}
		...
		GIVEN("as const void*")
		{
			THEN("returns false")
			{
				REQUIRE_FALSE(pred::notNullptr(static_cast<const void*>(nullptr)));
			}
		}
		...
		GIVEN("as int*")
		{
			THEN("returns false")
			{
				REQUIRE_FALSE(pred::notNullptr(static_cast<int*>(nullptr)));
			}
		}
		...
		GIVEN("as const int*")
		{
			THEN("returns false")
			{
				REQUIRE_FALSE(pred::notNullptr(static_cast<const int*>(nullptr)));
			}
		}
	}
}